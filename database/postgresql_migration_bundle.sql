-- =============================================================================
-- BEEKODING PLATFORM - POSTGRESQL & SUPABASE SQL MIGRATION BUNDLE
-- Compatible: PostgreSQL 12+, 13+, 14+, 15+, 16+, Supabase, Neon, pgAdmin, DBeaver
...
-- Nonaktifkan sementara pengecekan foreign key & triggers (Native PostgreSQL)
SET session_replication_role = 'replica';

-- -----------------------------------------------------------------------------
-- PEMBERSIHAN TABEL LAMA JIKA ADA (DROP TABLES)
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS class_batches (
    ...
    meet_url TEXT,
    session_dates_json TEXT,
    enrolled_students_json TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS class_attendance (
    ...
    notes TEXT,
    students_attendance_json TEXT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS curriculum_modules (
    ...
    sessions_plan_json TEXT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- -----------------------------------------------------------------------------
-- INDEKS PERFORMA QUERY (POSTGRESQL NATIVE)
-- -----------------------------------------------------------------------------

CREATE INDEX IF NOT EXISTS idx_audit_module ON audit_logs (module);
CREATE INDEX IF NOT EXISTS idx_audit_action ON audit_logs (action_type);
CREATE INDEX IF NOT EXISTS idx_audit_created_at ON audit_logs (created_at);
CREATE INDEX IF NOT EXISTS idx_question_tier_cat ON question_bank (tier, category);
CREATE INDEX IF NOT EXISTS idx_submission_parent_phone ON students_submissions (parent_phone);
CREATE INDEX IF NOT EXISTS idx_submission_status ON students_submissions (status);
CREATE INDEX IF NOT EXISTS idx_submission_created_at ON students_submissions (created_at);
CREATE INDEX IF NOT EXISTS idx_inquiry_status ON consultation_inquiries (status);
CREATE INDEX IF NOT EXISTS idx_inquiry_created_at ON consultation_inquiries (created_at);
CREATE INDEX IF NOT EXISTS idx_event_date ON coding_events (event_date);
CREATE INDEX IF NOT EXISTS idx_event_status ON coding_events (status);
CREATE INDEX IF NOT EXISTS idx_counseling_date ON counseling_sessions (session_date);
CREATE INDEX IF NOT EXISTS idx_counseling_status ON counseling_sessions (status);
CREATE INDEX IF NOT EXISTS idx_batch_status ON class_batches (status);
CREATE INDEX IF NOT EXISTS idx_attendance_date ON class_attendance (session_date);
CREATE INDEX IF NOT EXISTS idx_bundle_gamification_xp ON student_gamification (total_xp DESC);
CREATE INDEX IF NOT EXISTS idx_bundle_tx_status ON financial_transactions (payment_status);
CREATE INDEX IF NOT EXISTS idx_bundle_wa_status ON whatsapp_queued_messages (status);
CREATE INDEX IF NOT EXISTS idx_bundle_wa_trigger ON whatsapp_queued_messages (trigger_type);
...
-- Aktifkan kembali pengecekan foreign key & triggers (Native PostgreSQL)
