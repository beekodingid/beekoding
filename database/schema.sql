-- =============================================================================
-- BEEKODING EDUCATION PLATFORM - RELATIONAL DATABASE SCHEMA (DDL)
...
CREATE TABLE IF NOT EXISTS question_bank (
    ...
    options_json LONGTEXT NOT NULL, -- Array 4 pilihan jawaban [{id, text, score, explanation}]
...
CREATE TABLE IF NOT EXISTS students_submissions (
    ...
    scores_json LONGTEXT NOT NULL, -- {logical, numerical, spatial, pattern, creativity, problem_solving, language, persistence}
...
CREATE TABLE IF NOT EXISTS coding_events (
    ...
    description TEXT,
    tags_json LONGTEXT,
    participants_json LONGTEXT, -- Array data pendaftar event [{name, phone, email, registeredAt, status}]
...
CREATE TABLE IF NOT EXISTS instructors (
    ...
    skills_json LONGTEXT, -- Array keahlian ['Scratch', 'Python', 'Game Dev']
...
CREATE TABLE IF NOT EXISTS class_batches (
    ...
    meet_url TEXT,
    session_dates_json LONGTEXT, -- Array tanggal pelaksanaan 12 sesi
    enrolled_students_json LONGTEXT, -- Array siswa terdaftar [{id, studentName, parentName, parentPhone, enrolledAt}]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_batch_instructor FOREIGN KEY (instructor_id) REFERENCES instructors(id) ON DELETE SET NULL,
    INDEX idx_batch_status (status),
    INDEX idx_batch_instructor (instructor_id)
);
...
CREATE TABLE IF NOT EXISTS class_attendance (
    ...
    notes TEXT,
    students_attendance_json LONGTEXT NOT NULL, -- [{studentId, studentName, status: 'hadir'|'izin'|'sakit'|'alpa', note}]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_attendance_batch FOREIGN KEY (batch_id) REFERENCES class_batches(id) ON DELETE CASCADE,
    INDEX idx_attendance_batch_session (batch_id, session_number),
...
CREATE TABLE IF NOT EXISTS curriculum_modules (
    ...
    sessions_plan_json LONGTEXT NOT NULL, -- Array 12 sesi detail [{sessionNumber, title, topic, objectives, tools, assignment}]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS coding_quests (
    ...
    requirements_json LONGTEXT, -- Array kriteria tugas
...
CREATE TABLE IF NOT EXISTS quest_submissions (
    ...
    INDEX idx_submission_quest_student (quest_id, student_id)
);
...
CREATE TABLE IF NOT EXISTS quiz_exams (
    ...
    questions_json LONGTEXT NOT NULL, -- Array soal kuis pilihan ganda
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS student_gamification (
    ...
    badges_earned_json LONGTEXT, -- Array badge ID yang diperoleh
    last_activity_at TIMESTAMP NULL,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_gamification_xp (total_xp DESC)
);
...
CREATE TABLE IF NOT EXISTS student_certificates (
    ...
    INDEX idx_cert_student (student_name)
);
...
CREATE TABLE IF NOT EXISTS student_projects (
    ...
    parent_reviews_json LONGTEXT, -- Array ulasan orang tua
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS referral_ambassadors (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_ambassador_code (referral_code),
...
CREATE TABLE IF NOT EXISTS referral_records (
    ...
    INDEX idx_ref_ambassador_id (ambassador_id),
...
CREATE TABLE IF NOT EXISTS whatsapp_templates (
    ...
    variables_json LONGTEXT, -- Array variabel dinamis ['{nama_siswa}', '{jam_kelas}']
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS whatsapp_queued_messages (
    ...
    INDEX idx_wa_trigger (trigger_type),
    INDEX idx_wa_scheduled_at (scheduled_at)
);
